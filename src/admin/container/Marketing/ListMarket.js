...
const ListMarket = () => {
    ...
    const canAccessPage = hasPermission("Marketing"); // Check if the user can access the Marketing page
        ...
        return (
            <DataGrid
                ...
                getRowClassName={(params) => {
                    const liasoningItem = liasoningData.find(item => item.fillNo === params.row.fillNo);
                    return liasoningItem && liasoningItem.status === "Enabled" ? 'disabled-row' : '';
                }}
                autoHeight
                sx={{
                    "@media print": {
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            renderCell: (params) => {
                const liasoningItem = liasoningData.find(item => item.fillNo === params.row.fillNo);
                const isDisabled = liasoningItem && liasoningItem.status === "Enabled"; // Check if the row is disabled
                return (
                    <span className={isDisabled ? 'disabled-status' : ''}>
                        {isDisabled ? "Disabled" : params.row.status} {/* Change status to Disabled if row is disabled */}
                    </span>
                );
            }
        },
